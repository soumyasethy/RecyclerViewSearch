...
public class JJCircleToBarController extends JJBaseController {
  ...
  private void drawStartAnimView(Paint paint, Canvas canvas) {
    ...
    if (mPro <= 0.1f) {
      ...
      canvas.drawLine(mRectF2.left + cr + mCircleBig, mRectF.top, mRectF.right - cr - mCircleBig,
          mRectF.top, paint);
      canvas.drawLine(mRectF2.left + cr + mCircleBig, mRectF.bottom, mRectF.right - cr - mCircleBig,
          mRectF.bottom, paint);
      ...
      canvas.drawLine(cx - cr / 2 + 4, cy + cr / 2, cx - cr / 2 + 4 - cr / 2, cy - cr / 2 + 8,
          paint);
